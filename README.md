...
**当前阶段：**v6候选快照已验收（979 train / 112 dev，仍153条待审），无需重跑清洗。[E4规则先导](docs/E4_RULE_PILOT.md)已实现有官方来源的部分草案：24条件、11节点，102项离线测试通过，尚无真实模型效果。下一步做train侧可观测性和个案法律适用来源表，暂不需要用户运行API或GPU。[E3实际结果](docs/E3_PILOT_REPORT.md)仍未支持绑定增益，暂选flat；草案不自动复用旧条件或放行训练。
