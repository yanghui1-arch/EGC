...
**后续进度：**已完成修订规范后的200条试标，197条结构通过、3条拦截，并抽读固定随机30条。最新候选池为`data/cail_screened_v5`，隔离输入后保留1164条；不直接扩大当前联合提示。详见[200条审计报告](PILOT_200_REPORT.md)。下文首轮20条和v3池统计保留为历史记录。
...
- 据此已收紧profile到v2，训练池v3新增量刑建议过滤。当前八条件仍是案例级，事件/主体绑定是接下来必须验证的改进。200条后续试验已完成，结果见本文开头链接；尚未进行真实数据训练。
