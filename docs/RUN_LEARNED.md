...
**最新决定：下方v1重试命令先不继续运行。** 真实重试已定位到同句主体引用约束过严；按照用户建议转向[有上下文依据的主体恢复](CONTEXT_BINDING_DESIGN.md)，新schema尚未实现。日志/错误反馈/停止策略已修复并通过128项离线测试，但这不等于标注语义质量解决。保留现有数据、缓存和尝试历史，待新协议交付。
