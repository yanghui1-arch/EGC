# Learned Evidence v1：历史运行记录

**当前请使用[已实现的v2运行命令](RUN_LEARNED_V2.md)。下面全部是历史v1步骤，不要直接执行；当前代码默认目录已切换v2。** 新版允许Flash根据完整案情恢复主体，保留结构与来源检查；132项离线测试通过，无真实v2标注/训练结果。历史代码可在GitHub提交478a58b复现，旧缓存保持原样。
